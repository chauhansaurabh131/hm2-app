...
const NewDropDownTextInput = ({
  ...
  const [inputValue, setInputValue] = useState(value); // Local state to manage input value
  const [isFocused, setFocused] = useState(false);
  const refRBSheet = useRef(null);

  // Create animated state for the floating label
  const labelPosition = new Animated.Value(inputValue ? 1 : 0);

  useEffect(() => {
    // Sync the inputValue with the parent value
    if (value !== inputValue) {
      setInputValue(value);
    }
  }, [value]);
  ...
  const labelStyle = {
    ...
    color: isFocused || inputValue ? 'gray' : 'black', // Change label color on focus
  };
