...
const MatchesInAcceptedScreen = () => {
  ...
  const openModal = async item => {
    console.log(' === openModal___ ===> ', item?.friendList?._id);
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/api/v1/user/user/get-match-user/${item?.friendList?._id}`,
        {
          method: 'GET',
          headers: {
  ...
  const fetchData = async (pageNumber = 1) => {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/api/v1/user/friend/get-frd-mobile?page=${pageNumber}`,
        {
          method: 'GET',
          headers: {
            Authorization: `Bearer ${accessToken}`,
  ...
  const handleShare = async userItem => {
    ...
    const profileId = friendObj?._id || blockedFriendId;
        ...
        ListEmptyComponent={
          !loading && !isFetchingMore ? (
            <View style={style.emptyListContainer}>
              ...
              {/*      }}>*/}
              {/*  <Image source={icons.people_Icon} style={style.noImage} />*/}
        ...
        <View style={style.modalContainer}>
          <View style={style.modalBodyStyle}>
            ...
            <View style={style.matchNameContainer}>
              <Image source={icons.couple_icon} style={style.coupleIcons} />
