...
const MatchesInReceivedScreen = () => {
  ...
  const fetchData = async (pageNumber = 1) => {
    try {
      ...
      if (pageNumber === 1) {
        // 🔥 IMPORTANT — replace data on first page
        setData(newData);
      } else {
  ...
  const handleAccept = (userID, requestedId) => {
    dispatch(
      accepted_Decline_Request(
        {
          user: userId,
        ...
        () => {
          ...
          setData(prevData =>
            prevData.map(item =>
              item?._id === requestedId ? {...item, status: 'accepted'} : item,
            ),
          );
        },
      ),
...
export default MatchesInReceivedScreen;
